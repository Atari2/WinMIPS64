...
std::map<int, int> bht_predictor{};
std::unordered_map<int, std::string> addr_to_line{};
#define M_N_PRED_LINES 1024
#define M_V 2
#define N_V 2
constexpr uint32_t max_val_for_pred = (1 << N_V) - 1;
constexpr uint32_t halfway_for_pred = (1 << N_V) / 2;
constexpr uint32_t mask = (1 << M_V) - 1;
// (2, 2) mn predictor
struct MNPredictor {
	static uint32_t shift_reg;
	struct predictor {
		uint8_t preds[1 << M_V];
		uint32_t mispredictions = 0;
	};
	static uint32_t total_branches;
	static predictor predictors[M_N_PRED_LINES];

	static void update_pred(uint8_t& pred, bool taken, uint32_t& mis) {
		bool correct = taken ? (pred >= halfway_for_pred) : (pred < halfway_for_pred);
		if (!correct) {
			++mis;
		}
		if (taken && pred < max_val_for_pred) {
			++pred;
		}
	...
	static std::string pred_to_string(const predictor& pred) {
...
MNPredictor::predictor MNPredictor::predictors[M_N_PRED_LINES]{};
uint32_t MNPredictor::shift_reg{};
uint32_t MNPredictor::total_branches{};

#define BHT_N_BITS 2
constexpr int SAT_BHT_VAL = (1 << BHT_N_BITS) - 1;
constexpr int MIN_BHT_VAL = 0;
...
void insert_addr_line_mapping(char* line, int addr) {
	addr_to_line.insert({ addr, trim(std::string{line}) });
}
...
struct Console {
	Console() {
		if (!AllocConsole()) {
			OutputDebugString("Failed to allocate console\n");
		}
	}
	~Console() {
		FreeConsole();
...
static const Console init_dummy{};
...
#undef max
#undef min
	if (branch_status != NOT_A_BRANCH) {
		MNPredictor::branch(pipe->integer.NPC, branch_status == BRANCH_TAKEN);
		if (auto it = bht_predictor.find(pipe->integer.NPC); it != bht_predictor.end()) {
			if (branch_status == BRANCH_TAKEN) {
				it->second = std::min(SAT_BHT_VAL, it->second + 1);
			}
			else {
				it->second = std::max(MIN_BHT_VAL, it->second - 1);
			}
		}
		else {
			bht_predictor.insert({ pipe->integer.NPC, branch_status == BRANCH_TAKEN });
		}
	}
...
void clear_branch_map() {
	bht_predictor.clear();
	memset(MNPredictor::predictors, 0, sizeof(MNPredictor::predictors));
	MNPredictor::shift_reg = 0b00;
	MNPredictor::total_branches = 0;
	ClearConsole();
}
...
int clock_tick(pipeline* pipe, processor* mips, BOOL forwarding, BOOL delay_slot, BOOL branch_target_buffer, RESULT* result)
{
	...
	{ // check that pipeline is empty...
		...
		if (empty)
		{
			...
			std::string total{};
			auto format_mapping = [](const auto& p) {
				const auto& [addr, n_of_times] = p;
				if (auto it = addr_to_line.find(addr); it != addr_to_line.end()) {
					return std::format("BHT of branch {} (at address 0x{:04X}) at end is {}", it->second, addr, n_of_times);
				}
				else {
					return std::format("BHT of branch <invalid address-line mapping> (at address 0x{:04X}) at end is {}", addr, n_of_times);
				}
			};
			auto fmt = bht_predictor | v::transform(format_mapping) | v::join_with('\n') | r::to<std::string>();
			fmt += '\n';
			auto mispredicted = std::accumulate(std::begin(MNPredictor::predictors), std::end(MNPredictor::predictors), 0, [](int val, const MNPredictor::predictor& pred) {
